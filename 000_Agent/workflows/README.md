...
- **skills** 是「方法論 + SOP」，會被其他任務引用（例如備課技巧、試卷生成手冊）
- **workflows** 是「每天的固定儀式」，會**串接多個 skill** 一次跑完

所以一個 workflow 常常長這樣：
> `/morning` → 讀取最新教育新聞（用 email skill）→ 查今天課表與行事曆（用 calendar skill）→ 整理成備課與行政提醒（用 content-writing skill）

## 怎麼讓 workflow 變成 slash 指令？

workflow 檔案本身不在 Antigravity 自動掃描的位置，所以你需要**在全域的 commands 中配置對應的觸發器**：
