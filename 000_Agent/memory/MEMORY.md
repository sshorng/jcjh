...
- **ui-ux-pro-max-skill** (評估未安裝 · 2026-06-06)
  - **功能**：商業 UI/UX 設計系統生成器，提供各行業（如美容 Spa、SaaS）的配色、字型及排版建議。
  - **與現有 Skill 重合度**：與現有的 `claude-design`（提供設計導向、防 AI Slop、視覺規格等）在「優質網頁設計原則」上重疊。
  - **差異與互補**：其強項在於商業型 Landing Page 及 SaaS 介面之快速配置與無障礙（WCAG）檢測；而 `claude-design` 更偏向全方位網頁、簡報（Slide）與互動原型。
  - **夫子適用度評估**：偏低。夫子的核心需求在於「國文教學簡報、課堂互動 HTML、回顧影片製作」，而非商業品牌落地頁。若安裝會造成 Token 浪費與規則混淆，故決定**不予安裝**。
