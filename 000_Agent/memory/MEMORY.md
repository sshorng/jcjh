...
- **學思達講義補充資料偏好**：設計講義之 `【補充資料】` 時，**應儘量提供「國文閱讀、寫作技巧、修辭或文學分析等語文知識點」**以擴展學生學習（除非是特定的科普類課文，需要適度補充歷史或科技背景資訊），文字需高度精煉，且答案線索巧妙融入。
- **教師後台題庫匯入偏好**：教師後台「班級管理」裡的題庫按鈕，應明確處理「113 年度外題庫匯入」這種需求，不要只做「雲端缺漏比對」而讓按鈕看起來像沒作用。
- **未記錄變更提醒**：若夫子說「後面還有一些變更，但沒記進去」，要先把這件事視為新的操作線索，後續同步時要主動補記到 `MEMORY.md` 與當日日誌，避免脫節。
- **題庫載入策略**：學生端與題庫頁最好採「本地題庫 + Firestore 合併」而不是二選一，這樣雲端新增題目不會覆蓋掉本地既有年度資料。
- **教師後台版面偏好**：全域題庫操作要獨立成「題庫管理」區塊，不要塞在班級管理裡，避免語意混亂。
...
- **安裝新技能前的安全審查、查重與對比機制**：每當夫子要求安裝或建立新的技能 (Skill) 時，阿墨**必須**：
  1. **安全審查**：詳細排查該技能的腳本與代碼，檢查是否包含惡意系統指令、不安全的外部下載、或越權調用，確保系統安全無虞。
  2. **查重與比對**：主動比對現有技能清單，檢查是否有功能重疊或相似的舊技能。
  3. **呈報確認**：向夫子報告安全性與比較分析（差異性、互補性、必要性），在得到夫子許可後始得安裝，並將審查與分析紀錄寫入記憶與日誌中。
