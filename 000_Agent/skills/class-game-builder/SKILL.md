...
網頁簡報遊戲應包含以下互動關卡模組，將「深度文本探討」轉化為有趣的互動：

### 模組 A：正反心境對照卡片（文意理解）
* **情境**：左右分欄卡片，讓學生點擊對比兩個角色的心境或反應。
* **教學目的**：透過對立面的心境點選，讓學生體會人物在面臨絕境時的心理反差（例如慌亂 vs 鎮定）。

### 模組 B：虛實心防指數量表（認知與心理戰）
* **情境**：左側為防衛/部署按鈕（分為「他人作為」與「自己作為」分組），右側為「對手第一人稱主觀觀測鏡（內心獨白 OS）」。
* **教學目的**：學生點選不同部署，右側呈現對手看到此部署時的懷疑與震驚（疑慮值增長）。用以分析主角如何利用對手的「思維盲區」與「成見」進行心理戰。

### 模組 C：角色對話與寫作手法探索（寫作手法探討）
* **情境**：出現小說經典的角色對話插曲（如司馬昭質疑司馬懿）。
* **機制**：引導學生站在「大作家/編劇」的視角，點擊探索**「為什麼作者要在此處加入這段插曲？」**。
  - 按鈕分別代表不同的探索維度：【情節張力】、【人物反襯】、【側面烘托】。
  - 點擊後動態渲染該寫作手法的精緻分析，並解鎖翻頁。

### 模組 D：文本字詞尋寶（字詞與神態特寫）
* **情境**：在一段課文原文中，用 `<span>` 包裹單詞，讓學生用手指點選「最關鍵」或「最具打擊力」的神態特寫字詞。
* **機制**：點選字詞高亮，點選「驗證」按鈕。答對後（如選對神態關鍵詞）解鎖詳細的賞析面板。

### 模組 E：戰略設伏與地理沙盤（軍事預判與文意理解）
* **機制**：提供地圖路線或決策選項，答對解鎖伏兵突襲等 CSS 動畫特效，並引導學生理解主角如何精準預判對手的撤退路線。
...
## 🚀 交付流程
當觸發此技能時，請依照下列步驟進行：
1. 深入閱讀用戶提供的課文/大綱，提煉出 **3-5 個兼具文意理解與寫作美學的深度情境關卡**。
