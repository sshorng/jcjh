...
- Firestore 集合：安全收攏於 `slides/<slug>/wordcloud` 與 `slides/<slug>/polls/results` 下（避免不同簡報資料混用）
...
- Firestore 集合：安全收攏於 `slides/<slug>/wordcloud` 與 `slides/<slug>/polls/results`（每份簡報獨立集合）
